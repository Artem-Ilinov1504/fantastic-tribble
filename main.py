...
def add_teacher_and_student():
    ...
    name_t = request.form["name_t"]
    name_s = request.form["name_s"]
    new_teacher = Teacher(name=name_t)
    db.session.add(new_teacher)
    db.session.commit()
    new_student = Student(name=name_s)
    db.session.add(new_student)
    db.session.commit()
    return f"Teacher {new_teacher.name} and student {new_student.name} created successfully"
